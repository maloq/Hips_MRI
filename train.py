...
def run(args):
    #logdir = os.path.join('./logs',args.task) 
    ...
    n_augs = args.n_augs
    #writer = SummaryWriter(log_dir=logdir, comment=f'{args.plane}_AUG_{n_augs}_{now}')
    
    writer = SummaryWriter(comment=f'{args.task}_{args.plane}_AUG_{n_augs}')


    train_dataset = MRDataset(args.task, args.plane, train=True, drop_researh_tags=drop_researh_tags, researh_type=researh_types, resize=True, dim=(256, 256),
                 crop=True, transform=transform_flag, best_slice=True )

    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=1, shuffle=True, num_workers=10, drop_last=False)

    validation_dataset = MRDataset(args.task, args.plane, train=False, drop_researh_tags=drop_researh_tags, researh_type=researh_types, resize=True, dim=(256, 256),
                 crop=True, transform=False, best_slice=True )

    validation_loader = torch.utils.data.DataLoader(
        validation_dataset, batch_size=1, shuffle=-True, num_workers=10, drop_last=False)
    ...
    for epoch in range(num_epochs):
        ...
        if val_auc > 0.61 and train_auc > 0.61:
            ...
            if bool(args.save_model):
                ...
                torch.save(mrnet.state_dict(), f'./models/{file_name}')

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            iteration_change_loss = 0

        for tag, value in mrnet.named_parameters():
            tag = tag.replace('.', '/')
...
def parse_arguments():
    ...
    parser.add_argument('--prefix_name', type=str, default='Syn_AUG_toCAM')
    ...
    parser.add_argument('--load_path', type=str,
                        default='models/model_drop.pth')

    parser.add_argument('--n_augs', type=int,
                        default=1)
